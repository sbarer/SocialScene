...
app.get('/user', async (req,res)=>{
  ...
  var recent_reviews =  await client.query("select * from (select distinct on (id) * from review where email in (select f.email2 from friends f where f.email1 = '" + user.email + "' and '" + user.email + "' in (select ff.email2 from friends ff where ff.email1 != '" + user.email + "')) order by id, date_time ASC) as agg2 order by date_time ASC;")
